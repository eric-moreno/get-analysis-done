...
```bash
PHASE_INFO=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" find-phase "${PHASE}" --raw)
```
