...
<path_convention>
Wave summary output path convention: `wave-{N}/WAVE{N}_SUMMARY.md`

Each wave's lead analyst (or primary agent) writes the summary to `wave-{N}/WAVE{N}_SUMMARY.md` (e.g., `wave-1/WAVE1_SUMMARY.md`, `wave-6/WAVE6_SUMMARY.md`). The gate adapter in run-analysis.md step 4 reads from this path to extract gate metrics. All agents and templates MUST use this convention.

Wave report (separate from summary): `wave-{N}/WAVE_REPORT.md` — generated by run-wave after agent completion, contains execution metadata.
</path_convention>
...
**Note:** The WAVE_REPORT.md is distinct from the WAVE{N}_SUMMARY.md. The summary is produced by the lead analyst agent using the wave summary template (e.g., templates/WAVE6_SUMMARY.md) and contains the gate evaluation table. The report is produced by this workflow and contains execution metadata.
</step>
