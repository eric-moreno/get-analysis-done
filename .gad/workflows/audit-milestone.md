...
```bash
INIT=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" resolve-model gad-ml-specialist --raw)
```
...
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" phases list
```
...
PHASE_INFO=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="gad-ml-specialist",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
