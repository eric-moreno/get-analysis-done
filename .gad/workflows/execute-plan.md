...
@~/.claude/get-analysis-done/references/git-integration.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" init execute-phase "${PHASE}")
...
**Pattern A:** init_agent_tracking → spawn Task(subagent_type="gad-lead-analyst", model=executor_model) with prompt: execute plan at [path], autonomous, all tasks + SUMMARY + commit, follow deviation/auth rules, report: plan name, tasks, SUMMARY path, commit hash → track agent_id → wait → update tracking → report.
...
2. Per segment:
   - Subagent route: spawn gad-lead-analyst for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" phases list --type summaries --raw
...
See `~/.claude/get-analysis-done/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/get-analysis-done/references/checkpoints.md for details.
</step>
...
If user_setup exists: create `{phase}-USER-SETUP.md` using template `~/.claude/get-analysis-done/templates/user-setup.md`. Per service: env vars table, account setup checklist, dashboard config, local dev notes, verification commands. Status "Incomplete". Set `USER_SETUP_CREATED=true`. If empty/missing: skip.
</step>

<step name="create_summary">
Create `{phase}-{plan}-SUMMARY.md` at `.planning/phases/XX-name/`. Use `~/.claude/get-analysis-done/templates/summary.md`.
...
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" state update-progress

# Record execution metrics
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" state record-metric \
...
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" state add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" state record-session \
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
</step>
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
