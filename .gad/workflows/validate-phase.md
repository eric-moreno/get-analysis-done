...
```bash
AUDITOR_MODEL=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" resolve-model gad-note-writer --raw)
NYQUIST_CFG=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
**State B (create):**
1. Read template from `~/.claude/get-analysis-done/templates/VALIDATION.md`
...
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" commit-docs "docs(phase-${PHASE}): add/update validation strategy"
```
