<purpose>
Switch the model profile used by GAD agents. Controls which Claude model each agent uses, balancing quality vs token spend.
</purpose>
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" config-ensure-section
...
[Show table from MODEL_PROFILES in gad-tools.cjs for selected profile]

Example:
| Agent | Model |
|-------|-------|
| gad-lead-analyst | opus |
| gad-lead-analyst | sonnet |
| gad-cross-checker | haiku |
| ... | ... |
