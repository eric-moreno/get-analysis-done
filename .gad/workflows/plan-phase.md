<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates gad-theory-scout, gad-lead-analyst, and gad-cross-checker agents with a revision loop (max 3 iterations).
</purpose>
...
```bash
INIT=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" roadmap get-phase "${PHASE}")
```
...
5. Commit:
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
If "Run discuss-phase first": Display `/gad:discuss-phase {X}` and exit workflow.
...
### Spawn gad-theory-scout

```bash
PHASE_DESC=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" roadmap get-phase "${PHASE}" | jq -r '.section')
```
...
Task(
  ...
  subagent_type="gad-theory-scout",
...
**If found:**
1. Read template: `~/.claude/get-analysis-done/templates/VALIDATION.md`
...
1. Re-run: `/gad:plan-phase {PHASE} --research`
2. Disable Nyquist in config
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 8 only if user selects 2 or 3.

## 8. Spawn gad-lead-analyst Agent
...
## 10. Spawn gad-cross-checker Agent
...
```
Skill(skill="gad:execute-phase", args="${PHASE} --auto --no-transition")
```
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Next: /gad:discuss-phase ${NEXT_PHASE} --auto
  ```
...
- [ ] gad-theory-scout spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] gad-lead-analyst spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] gad-cross-checker spawned with CONTEXT.md
