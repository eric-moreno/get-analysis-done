...
for dir in .claude .config/opencode .opencode .gemini; do
  if [ -f "./$dir/get-analysis-done/VERSION" ]; then
    LOCAL_VERSION_FILE="./$dir/get-analysis-done/VERSION"
    LOCAL_MARKER_FILE="./$dir/get-analysis-done/workflows/update.md"
...
for dir in .claude .config/opencode .opencode .gemini; do
  if [ -f "$HOME/$dir/get-analysis-done/VERSION" ]; then
    GLOBAL_VERSION_FILE="$HOME/$dir/get-analysis-done/VERSION"
    GLOBAL_MARKER_FILE="$HOME/$dir/get-analysis-done/workflows/update.md"
...
```bash
npm view get-analysis-done-cc version 2>/dev/null
```
...
To update manually: `npx get-analysis-done-cc --global`
```
...
- `commands/gad/` will be wiped and replaced
- `get-analysis-done/` will be wiped and replaced
...
- Custom commands not in `commands/gad/` ✓
...
**If LOCAL install:**
```bash
npx -y get-analysis-done-cc@latest --local
```

**If GLOBAL install (or unknown):**
```bash
npx -y get-analysis-done-cc@latest --global
```
...
[View full changelog](https://github.com/glittercowboy/get-analysis-done/blob/main/CHANGELOG.md)
```
</step>
