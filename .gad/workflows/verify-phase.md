...
<required_reading>
@~/.claude/get-analysis-done/references/verification-patterns.md
@~/.claude/get-analysis-done/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" init phase-op "${PHASE_ARG}")
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" roadmap get-phase "${phase_number}"
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" verify key-links "$plan")
...
See ~/.claude/get-analysis-done/templates/verification-report.md for complete template.
</step>
