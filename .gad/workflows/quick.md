<purpose>
Execute small, ad-hoc tasks with GAD guarantees (atomic commits, STATE.md tracking). Quick mode spawns gad-lead-analyst (quick mode) + gad-lead-analyst(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```bash
INIT=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" init quick "$DESCRIPTION")
...
Spawn gad-lead-analyst with plan reference:
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
...
GAD > QUICK TASK COMPLETE (FULL MODE)
