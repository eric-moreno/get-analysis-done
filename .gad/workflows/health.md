...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" validate health $REPAIR_FLAG
```
...
- [W001] STATE.md references phase 5, but only phases 1-3 exist
  Fix: Run /gad:health --repair to regenerate
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" validate health
```
