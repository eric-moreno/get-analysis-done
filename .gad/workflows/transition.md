...
```bash
TRANSITION=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" phase complete "${current_phase}")
```
...
```bash
PROGRESS=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" progress bar --raw)
```
...
Exit skill and invoke SlashCommand("/gad:plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/gad:discuss-phase [X+1] --auto")
...
`/gad:discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/gad:plan-phase [X+1]` — skip discussion, plan directly
- `/gad:research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/gad:discuss-phase [X+1]` — revisit context
- `/gad:research-phase [X+1]` — investigate unknowns
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" config-set workflow._auto_chain_active false
```
...
Exit skill and invoke SlashCommand("/gad:complete-milestone {version}")
