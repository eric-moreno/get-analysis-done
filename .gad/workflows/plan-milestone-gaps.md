...
PHASES=$(node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" phases list)
...
```bash
node "$HOME/.claude/get-analysis-done/bin/gad-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
