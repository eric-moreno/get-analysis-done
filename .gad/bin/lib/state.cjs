...
function buildStateFrontmatter(bodyContent, cwd) {
  ...
  const fm = { gad_state_version: '1.0' };
  ...
  // GAD-specific: blinding and wave fields
  const blindingStatus = stateExtractField(bodyContent, 'Blinding Status')
    || stateExtractField(bodyContent, 'blinding_status');
  const currentWave = stateExtractField(bodyContent, 'Current Wave');
  const gateRetriesRemaining = stateExtractField(bodyContent, 'Gate Retries Remaining');
  const reblindCount = stateExtractField(bodyContent, 'Reblind Count');

  if (blindingStatus) fm.blinding_status = blindingStatus;
  if (currentWave !== null && currentWave !== undefined) fm.current_wave = parseInt(currentWave, 10) || 0;
  if (gateRetriesRemaining !== null && gateRetriesRemaining !== undefined) fm.gate_retries_remaining = parseInt(gateRetriesRemaining, 10);
  if (reblindCount !== null && reblindCount !== undefined) fm.reblind_count = parseInt(reblindCount, 10);
...
// ─── GAD Blinding & Wave Helpers ─────────────────────────────────────────────

/**
 * Valid blinding state transitions per the staged unblinding protocol.
 * blinded -> sidebands -> partial_10pct -> unblinded
 * With re-blinding paths: sidebands->blinded, partial_10pct->blinded,
 * unblinded->blinded (only if reblind_count < 1)
 */
const VALID_BLINDING_TRANSITIONS = {
  blinded: ['sidebands'],
  sidebands: ['partial_10pct', 'blinded'],
  partial_10pct: ['unblinded', 'blinded'],
  unblinded: ['blinded'],
};

const VALID_BLINDING_STATES = ['blinded', 'sidebands', 'partial_10pct', 'unblinded'];

function getBlindingStatus(cwd) {
  const statePath = path.join(cwd, '.planning', 'STATE.md');
  if (!fs.existsSync(statePath)) return 'blinded';
  const content = fs.readFileSync(statePath, 'utf-8');
  const status = stateExtractField(content, 'Blinding Status')
    || stateExtractField(content, 'blinding_status');
  return status || 'blinded';
}

function setBlindingStatus(cwd, newStatus) {
  if (!VALID_BLINDING_STATES.includes(newStatus)) {
    throw new Error('Invalid blinding status: ' + newStatus + '. Valid: ' + VALID_BLINDING_STATES.join(', '));
  }
  ...
  let content = fs.readFileSync(statePath, 'utf-8');
  const currentStatus = stateExtractField(content, 'Blinding Status')
    || stateExtractField(content, 'blinding_status')
    || 'blinded';

  // Validate transition
  const allowedTransitions = VALID_BLINDING_TRANSITIONS[currentStatus];
  if (!allowedTransitions || !allowedTransitions.includes(newStatus)) {
    throw new Error(
      'Invalid blinding transition: ' + currentStatus + ' -> ' + newStatus +
      '. Allowed from ' + currentStatus + ': ' + (allowedTransitions || []).join(', ')
    );
  }

  // Special case: re-blinding from unblinded requires reblind_count < 1
  if (currentStatus === 'unblinded' && newStatus === 'blinded') {
    const reblindCountRaw = stateExtractField(content, 'Reblind Count');
    const reblindCount = parseInt(reblindCountRaw, 10) || 0;
    if (reblindCount >= 1) {
      throw new Error(
        'Cannot re-blind: reblind_count is ' + reblindCount + ' (max 1 re-blinding allowed)'
      );
    }
    // Increment reblind count
    const result = stateReplaceField(content, 'Reblind Count', String(reblindCount + 1));
    if (result) content = result;
  }

  // Update blinding status field
  let updated = stateReplaceField(content, 'Blinding Status', newStatus);
  if (!updated) {
    updated = stateReplaceField(content, 'blinding_status', newStatus);
  }
  if (!updated) {
    throw new Error('Could not find Blinding Status field in STATE.md');
  }
...
function getCurrentWave(cwd) {
  const statePath = path.join(cwd, '.planning', 'STATE.md');
  if (!fs.existsSync(statePath)) return 0;
  const content = fs.readFileSync(statePath, 'utf-8');
  const wave = stateExtractField(content, 'Current Wave');
  return wave !== null ? (parseInt(wave, 10) || 0) : 0;
}

function setCurrentWave(cwd, waveNumber) {
  const wave = parseInt(waveNumber, 10);
  if (isNaN(wave) || wave < 0 || wave > 7) {
    throw new Error('Invalid wave number: ' + waveNumber + '. Must be 0-7.');
  }
  ...
  let content = fs.readFileSync(statePath, 'utf-8');
  const updated = stateReplaceField(content, 'Current Wave', String(wave));
  if (!updated) {
    throw new Error('Could not find Current Wave field in STATE.md');
  }
...
module.exports = {
  ...
  getCurrentWave,
  setCurrentWave,
  VALID_BLINDING_TRANSITIONS,
  VALID_BLINDING_STATES,
  cmdStateLoad,
  cmdStateGet,
  cmdStatePatch,
  cmdStateUpdate,
