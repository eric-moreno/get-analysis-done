/**
 * Wave — Wave/Phase CRUD, query, lifecycle operations, and wave artifact commits
 *
 * Renamed from phase.cjs for the GAD analysis execution model.
 * Retains phase lifecycle functions for .planning/ infrastructure.
 * Adds wave-specific operations for HEP analysis execution (waves 0-7).
 */

const fs = require('fs');
const path = require('path');
const { escapeRegex, normalizePhaseName, comparePhaseNum, findPhaseInternal, getArchivedPhaseDirs, generateSlugInternal, getMilestonePhaseFilter, toPosixPath, execGit, output, error } = require('./core.cjs');
...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  // Build phase entry
  const phaseEntry = `\n### Phase ${newPhaseNum}: ${description}\n\n**Goal:** [To be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${maxPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /gad:plan-phase ${newPhaseNum} to break down)\n`;
...
function cmdPhaseInsert(cwd, afterPhase, description, raw) {
  ...
  // Build phase entry
  const phaseEntry = `\n### Phase ${decimalPhase}: ${description} (INSERTED)\n\n**Goal:** [Urgent work - to be planned]\n**Requirements**: TBD\n**Depends on:** Phase ${afterPhase}\n**Plans:** 0 plans\n\nPlans:\n- [ ] TBD (run /gad:plan-phase ${decimalPhase} to break down)\n`;
...
// ─── Wave Artifact Commit ────────────────────────────────────────────────────

/**
 * Stage and atomically commit all artifacts for a completed wave.
 * Uses the existing execGit() utility from core.cjs.
 *
 * @param {string} cwd - Project root directory
 * @param {number} waveNumber - Wave number (0-7)
 * @param {string[]} artifactPaths - Relative paths of artifacts to commit
 * @returns {{ exitCode: number, stdout: string, stderr: string }}
 */
function commitWaveArtifacts(cwd, waveNumber, artifactPaths) {
  if (!Array.isArray(artifactPaths) || artifactPaths.length === 0) {
    return { exitCode: 1, stdout: '', stderr: 'No artifact paths provided' };
  }

  // Stage all wave artifacts
  for (var i = 0; i < artifactPaths.length; i++) {
    var addResult = execGit(cwd, ['add', artifactPaths[i]]);
    if (addResult.exitCode !== 0) {
      return { exitCode: addResult.exitCode, stdout: addResult.stdout, stderr: 'Failed to stage: ' + artifactPaths[i] + ' - ' + addResult.stderr };
    }
  }

  // Atomic commit with wave metadata
  var message = 'wave(' + waveNumber + '): complete wave ' + waveNumber + ' artifacts\n\n' +
    'Files: ' + artifactPaths.length + ' artifacts committed\n' +
    'Wave: ' + waveNumber;
