 ...
 *
 * Replaces repetitive inline bash patterns across ~50 GAD command/workflow/agent files.
 * Centralizes: config parsing, model resolution, phase lookup, git commits, summary verification.
 *
 * Usage: node gad-tools.cjs <command> [args] [--raw]
 *
 * Atomic Commands:
...
const phase = require('./lib/wave.cjs');
...
async function main() {
  ...
  if (!command) {
    error('Usage: gad-tools <command> [args] [--raw] [--cwd <path>]\nCommands: state, resolve-model, find-phase, commit, verify-summary, verify, frontmatter, template, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section, init');
  }
